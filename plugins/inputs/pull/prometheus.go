...
import (
	...
	"Dana/internal"
	"Dana/models"
	common_http "Dana/plugins/common/http"
	"Dana/plugins/inputs"
	"Dana/plugins/parsers/openmetrics"
	parsers_prometheus "Dana/plugins/parsers/prometheus"
)
	...
	if p.HTTPClientConfig.ResponseHeaderTimeout != 0 {
		p.Log.Warn(
			"Config option response_timeout was set to non-zero value. This option's behavior was " +
				"changed in Dana2 1.30.2 and now controls the HTTP client's header timeout and " +
